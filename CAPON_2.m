...
num_trials = 10;                % 蒙特卡洛实验次数，进行100次实验来平均化结果
...
figure;                   % 创建新图形窗口
hold on;                  % 保持当前图形，不覆盖
plot(snr_values, RMSE(:, 1), '-o', 'DisplayName', 'CAPON', 'LineWidth', 1.5);  % 绘制信噪比与RMSE的关系曲线，使用圆点标记
xlabel('SNR (dB)');       % x轴标签：信噪比（dB）
ylabel('RMSE');           % y轴标签：均方根误差（RMSE）
grid on;                  % 打开网格
legend;                   % 显示图例
title('不同信噪比下CAPON方法RMSE评估');  % 图表标题：DOA估计的RMSE
