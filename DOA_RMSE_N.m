...
RMSE = zeros(length(N_values), 6); % 存储平均RMSE
...
figure;
hold on;
plot(N_values, RMSE(:, 1), '-o', 'DisplayName', 'MUSIC', 'LineWidth', 1.5);
plot(N_values, RMSE(:, 2), '-*', 'DisplayName', 'ROOT-MUSIC', 'LineWidth', 1.5);
plot(N_values, RMSE(:, 3), '-s', 'DisplayName', 'LS-ESPRIT', 'LineWidth', 1.5);
plot(N_values, RMSE(:, 4), '-^', 'DisplayName', 'TLS-ESPRIT', 'LineWidth', 1.5);
plot(N_values, RMSE(:, 5), '-d', 'DisplayName', 'CAPON', 'LineWidth', 1.5);
plot(N_values, RMSE(:, 6), '->', 'DisplayName', 'DML', 'LineWidth', 1.5);
xlabel('N'); ylabel('RMSE'); grid on;
legend; title('不同快拍数下DOA估计方法对比');
xlim([64, 1024]);
box on;

axes('Position',[0.3 0.6 0.25 0.25]); % 定义子图位置和大小
box on;  %显示子图的外框线
hold on;
plot(N_values, RMSE(:, 1), '-o', 'DisplayName', 'MUSIC', 'LineWidth', 1.5);
plot(N_values, RMSE(:, 2), '-*', 'DisplayName', 'ROOT-MUSIC', 'LineWidth', 1.5);
