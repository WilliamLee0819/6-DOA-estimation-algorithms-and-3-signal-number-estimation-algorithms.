...
%% 迭代
for idx = 1:length(N_values)   % 循环遍历每一个快拍数
    N = N_values(idx);       % 当前
    ...
    for trial = 1:num_trials     % 进行num_trials次蒙特卡洛实验
        ...
        est_DOA_music=sort(theta_rootmusic);
...
hold on;                  % 保持当前图形，不覆盖
plot(N_values, RMSE(:, 1), '-o', 'DisplayName', 'ROOT-MUSIC', 'LineWidth', 1.5);  % 绘制快拍数与RMSE的关系曲线，使用圆点标记
xlabel('N');       
ylabel('RMSE');           % y轴标签：均方根误差（RMSE），字体大小14
grid on;                  % 打开网格
legend;   % 显示图例，字体大小14
title('不同快拍数下ROOT-MUSIC方法RMSE评估');  % 图表标题：DOA估计的RMSE，字体大小14
