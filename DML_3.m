...
num_trials = 1000;                % 蒙特卡洛实验次数，进行100次实验来平均化结果
...
hold on;                  % 保持当前图形，不覆盖
plot(N_values, RMSE(:, 1), '-o', 'DisplayName', 'DML', 'LineWidth', 1.5);  % 绘制快拍数与RMSE的关系曲线，使用圆点标记
xlabel('N');      
ylabel('RMSE');           % y轴标签：均方根误差（RMSE），字体大小14
grid on;                  % 打开网格
legend;   % 显示图例，字体大小14
title('不同快拍数下DML方法RMSE评估');  % 图表标题：DOA估计的RMSE，字体大小14
